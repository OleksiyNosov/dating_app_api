...
RSpec.describe Api::PlacesController, type: :controller do
  ...
  describe '#index' do
    let(:params) { { city: 'London', tags: %w[beer pizza], range: '6' } }
  ...
  describe '#resource' do
    context 'place is not nil' do
      xit {}
    end

    context 'place is nil' do
      context 'params[:id] contains not zero integer' do
        let(:params) { '1' }
        ...
        before do
          ...
          expect(subject).to receive(:params) do
            double.tap do |a|
              expect(a).to receive(:[]).with(:id) do
                double.tap do |b|
                  expect(b).to receive(:to_i) do
                    double.tap { |c| expect(c).to receive(:zero?).and_return false }
                  end
                end
              end
            end
      ...
      context 'params[:id] contains string or zero' do
        context 'place found by city' do
          let(:params) { 'Tallinn' }
          ...
          before do
            ...
            expect(subject).to receive(:params) do
              double.tap do |a|
                expect(a).to receive(:[]).with(:id) do
                  double.tap do |b|
                    expect(b).to receive(:to_i) do
                      double.tap { |c| expect(c).to receive(:zero?).and_return true }
                    end
                  end
                end
              end
            ...
            before { expect(Place).to receive(:find_by).with(city: params).and_return place }

            its(:resource) { is_expected.to eq place }
          end
        end

        context 'place generated by city' do
          let(:params) { 'Tallinn' }
          ...
          before do
            ...
            before do
              #
              # => PlaceApiGenerator.new(params[:id]).download_and_create_place
              #
              expect(PlaceApiGenerator).to receive(:new) do
                double.tap { |a| expect(a).to receive(:download_and_create_place).and_return place }
              end
            end
